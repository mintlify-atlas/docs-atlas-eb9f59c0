---
title: "Medical History Controllers: Visits, Allergies, Vaccines, and More"
sidebarTitle: "Medical History"
description: "Reference for VisitsController, AllergiesController, VaccinesController, GrowthChartController, PermMedController, PersonalBackgroundController, and PerinatalBackgroundController endpoints."
---

Turnero organises a patient's clinical history across seven specialised controllers, each responsible for a distinct category of medical data. Every controller in this group requires the `Medico` role. All list endpoints follow the same DataTables server-side pattern: a `GET` action renders a partial view containing an empty `<table>` placeholder, and a companion `POST` initialisation endpoint provides the JSON data feed. Create and edit forms are delivered as partial views embedded inside the patient detail page, keeping the UI entirely Ajax-driven without full page reloads.
...
| Method | Path | Roles | Description |
|--------|------|-------|-------------|
| `GET` | `/Allergies/Index/{id}` | Medico | Returns `_AllergiesTable` partial |
| `POST` | `/Allergies/InitializeAllergies` | Medico | DataTables data feed |
| `GET` | `/Allergies/Create/{id}` | Medico | Returns `_Create` partial |
| `POST` | `/Allergies/Create` | Medico | Inserts a new allergy |
| `GET` | `/Allergies/Edit/{id}` | Medico | Returns `_Create` partial pre-filled for editing |
| `POST` | `/Allergies/Edit` | Medico | Updates an allergy |
| `DELETE` | `/Allergies/Delete/{id}` | Medico | Deletes an allergy |
| `GET` | `/Allergies/Details/{id}` | Medico | Returns `_Details` partial for one allergy |
